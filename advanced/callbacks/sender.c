...
void logger(int level, char * identifier, char * message, struct MessageBlock * udata) {

    printf("%d, %s, %s, %d\n", level, identifier, message, udata->x);
...
int main() {
  ...
  helicsFederateInfoSetIntegerProperty(fedinfo, helics_property_int_log_level, 9, &err);
  ...
  pub = helicsFederateRegisterGlobalPublication(vfed, "testA", helics_data_type_string, "",&err);
  ...
  struct MessageBlock userdata = {.x = 5};

  helicsFederateSetLoggingCallback(vfed, logger, &userdata);
  ...
  */
  ...
  for (int i = 0; i <= 10; i++)
    {
        currenttime=helicsFederateRequestTime(vfed, i, &err);
        ...
        helicsFederateLogInfoMessage(vfed, "test MEXAGE", &err);

        if (err.error_code!=helics_ok)
        {
            fprintf(stderr, "HELICS log failed:%s\n",err.message);
        }
